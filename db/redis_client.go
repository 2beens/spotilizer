...
func SaveFavTracksSnapshot(ft *m.FavTracksSnapshot) (saved bool) {
...
func GetFavTrakcsSnapshot(key string) *m.FavTracksSnapshot {
	cmd := rc.Get(key)
	if err := cmd.Err(); err != nil && err != redis.Nil {
...
func GetAllFavTracksSnapshots(username string) *[]m.FavTracksSnapshot {
